...
def fast_hist1(label_pred, label_true, sem_mask, num_classes):
    mask = (sem_mask==255) & ((label_true == 13) | (label_true == 16) | (label_true == 17) )
    #num_classes=19
    #mask = ((label_pred == 11) | (label_pred == 13) | (label_pred == 14) | (label_pred == 15) | (label_pred == 16) | (label_pred == 17) | (label_pred == 18))
    hist = np.bincount(
...
def evaluate_eval(args, net, optimizer, val_loss, hist, dump_images, writer, epoch=0, dataset=None, ):
    ...
    to_save_dir = os.path.join(args.exp_path,"models_NormAudio_noskip_logmap_diffmask_Comp_3class_noBG_nodiff_mic38_flipped");os.makedirs(to_save_dir, exist_ok=True)
    save_snapshot = 'SOP_epoch_{}.pth'.format(epoch)
    ...
    if mean_iu > args.best_record['mean_iu'] :
        ...
        best_snapshot = 'best_epoch_{}_mean-iu_{:.5f}.pth'.format(
            args.best_record['epoch'], args.best_record['mean_iu'])
        best_snapshot = os.path.join(args.exp_path, best_snapshot)
        shutil.copyfile(save_snapshot, best_snapshot)
        
    
    to_save_dir = os.path.join(args.exp_path, 'best_images_NormAudio_noskip_logmap_diffmask_Comp_3class_noBG_nodiff_mic38_flipped')
    ...
    for bs_idx, bs_data in enumerate(dump_images):
        for local_idx, data in enumerate(zip(bs_data[0], bs_data[1],bs_data[2])):
            ...
            prediction_fn = str(epoch)+'_{}_prediction.png'.format(img_name)
            predictions_pil.save(os.path.join(to_save_dir, prediction_fn))
            gt_fn = str(epoch)+'_{}_gt.png'.format(img_name)
