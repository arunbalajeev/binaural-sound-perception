...
parser.add_argument('--arch', type=str, default='network.deepv3_noBG_Paralleltask_depth.DeepWV3Plus',
                    help='Network architecture. We have DeepSRNX50V3PlusD (backbone: ResNeXt50) \
                    and deepWV3Plus (backbone: WideResNet38).')
parser.add_argument('--dataset', type=str, default='OmniAudio_noBG_Paralleltask_depth',
...
parser.add_argument('--snapshot', type=str, default=None)#"logs/ckpt/default/Omni-network.deepv3_audioBG_Spec_diffmask_Comp_noBG_Paralleltask_depth.DeepWV3Plus/models_depth_10_005depthweight_mic47/SOP_epoch_3.pth")#"/srv/beegfs02/scratch/language_vision/data/Sound_Event_Prediction/semantic-segmentation-master/pretrained_models/cityscapes_best_wideresnet38.pth")#
...
def train(train_loader, net, optim, curr_epoch, writer):
    ...
    for i, data in enumerate(train_loader):
        # inputs = (2,3,713,713)
        # gts    = (2,713,713)
        in_imgs, gts,gts_depth, gts_diff_2, gts_diff_5, sem_mask, in_aud1, in_aud6, _img_name = data

        batch_pixel_size = in_imgs.size(0) * in_imgs.size(2) * in_imgs.size(3)
        #print(in_imgs.size())
        in_imgs, gts,gts_depth, gts_diff_2, gts_diff_5, in_aud1, in_aud6 = in_imgs.type(torch.FloatTensor).cuda(), gts.cuda(),gts_depth.type(torch.FloatTensor).cuda(),gts_diff_2.cuda(),gts_diff_5.cuda(), in_aud1.type(torch.FloatTensor).cuda(), in_aud6.type(torch.FloatTensor).cuda()
        ...
        main_loss = net(in_imgs, in_aud1, in_aud6, gts=gts,gts_diff_2=gts_diff_2,gts_diff_5=gts_diff_5,gts_depth=gts_depth)
        #print(main_loss)
        if args.apex:
...
def validate(val_loader, net, criterion, optim, curr_epoch, writer):
    ...
    for val_idx, data in enumerate(val_loader):
        ...
        in_imgs, gt_image,gts_depth,gts_diff_2, gts_diff_5,sem_mask, in_aud1, in_aud6, img_names = data
        assert len(in_imgs.size()) == 4 and len(gt_image.size()) == 3
        #assert inputs.size()[2:] == gt_image.size()[1:]

        batch_pixel_size = in_imgs.size(0) * in_imgs.size(2) * in_imgs.size(3)
        in_imgs, gt_cuda,gts_depth,gts_diff_2, gts_diff_5,sem_mask, in_aud1, in_aud6 = in_imgs.type(torch.FloatTensor).cuda(), gt_image.cuda(),gts_depth.type(torch.FloatTensor).cuda(),gts_diff_2.cuda(),gts_diff_5.cuda(),sem_mask.cuda(), in_aud1.type(torch.FloatTensor).cuda(), in_aud6.type(torch.FloatTensor).cuda()
        #inputs, gt_cuda = inputs.cuda(), gt_image.cuda()

        with torch.no_grad():
            output,outdepth  = net(in_imgs, in_aud1, in_aud6, gts=gt_cuda,gts_diff_2=gts_diff_2,gts_diff_5=gts_diff_5,gts_depth=gts_depth)  # output = (1, 19, 713, 713)
        ...
        if val_idx%10==0:
            ...
            # Saving numpy file
            output_directory = "/srv/beegfs02/scratch/language_vision/data/Sound_Event_Prediction/audio/semanticPred/logs/ckpt/default/Omni-network.deepv3_audioBG_Spec_diffmask_Comp_noBG_Paralleltask_depth.DeepWV3Plus/";os.makedirs(output_directory+"best_depths_10_005depthweight_mic25/", exist_ok=True)
            #output_name = os.path.splitext(os.path.basename(image_path))[0];output_name=output_name.split("_")[-1];
            name_dest_npy = os.path.join(output_directory,"best_depths_10_005depthweight_mic25/", "{}_disp.npy".format(img_name))
            ...
            name_dest_im = os.path.join(output_directory,"best_depths_10_005depthweight_mic25/", str(curr_epoch)+"_{}_disp.jpg".format(img_name))
    ...
    to_save_dir = os.path.join(output_directory,"models_depth_10_005depthweight_mic25/");os.makedirs(to_save_dir, exist_ok=True)
