...
parser.add_argument('--arch', type=str, default='network.deepv3_noBG_Paralleltask.DeepWV3Plus',
                    help='Network architecture. We have DeepSRNX50V3PlusD (backbone: ResNeXt50) \
                    and deepWV3Plus (backbone: WideResNet38).')
parser.add_argument('--dataset', type=str, default='OmniAudio_noBG_Paralleltask',
...
parser.add_argument('--snapshot', type=str, default=None)#"/srv/beegfs02/scratch/language_vision/data/Sound_Event_Prediction/audio/semanticPred/logs/ckpt/default/Omni-network.deepv3_audioBG_Spec_diffmask_Comp_noBG_Paralleltask.DeepWV3Plus/models_NormAudio_noskip_logmap_diffmask_Comp_3class_noBG_nodiff_mic16_flipped/SOP_epoch_8.pth")#"/srv/beegfs02/scratch/language_vision/data/Sound_Event_Prediction/semantic-segmentation-master/pretrained_models/cityscapes_best_wideresnet38.pth")#
...
def train(train_loader, net, optim, curr_epoch, writer):
    ...
    for i, data in enumerate(train_loader):
        # inputs = (2,3,713,713)
        # gts    = (2,713,713)
        in_imgs, gts, gts_diff_2, gts_diff_5, sem_mask, in_aud1, in_aud6, _img_name = data

        batch_pixel_size = in_imgs.size(0) * in_imgs.size(2) * in_imgs.size(3)
        #print(in_imgs.size())
        in_imgs, gts, gts_diff_2, gts_diff_5, in_aud1, in_aud6 = in_imgs.type(torch.FloatTensor).cuda(), gts.cuda(),gts_diff_2.cuda(),gts_diff_5.cuda(), in_aud1.type(torch.FloatTensor).cuda(), in_aud6.type(torch.FloatTensor).cuda()
        ...
        main_loss = net(in_imgs, in_aud1, in_aud6, gts=gts,gts_diff_2=gts_diff_2,gts_diff_5=gts_diff_5)
        #print(main_loss)
        if args.apex:
...
def validate(val_loader, net, criterion, optim, curr_epoch, writer):
    ...
    for val_idx, data in enumerate(val_loader):
        ...
        in_imgs, gt_image,gts_diff_2, gts_diff_5,sem_mask, in_aud1, in_aud6, img_names = data
        assert len(in_imgs.size()) == 4 and len(gt_image.size()) == 3
        #assert inputs.size()[2:] == gt_image.size()[1:]

        batch_pixel_size = in_imgs.size(0) * in_imgs.size(2) * in_imgs.size(3)
        in_imgs, gt_cuda,gts_diff_2, gts_diff_5,sem_mask, in_aud1, in_aud6 = in_imgs.type(torch.FloatTensor).cuda(), gt_image.cuda(),gts_diff_2.cuda(),gts_diff_5.cuda(),sem_mask.cuda(), in_aud1.type(torch.FloatTensor).cuda(), in_aud6.type(torch.FloatTensor).cuda()
        #inputs, gt_cuda = inputs.cuda(), gt_image.cuda()

        with torch.no_grad():
            output = net(in_imgs, in_aud1, in_aud6, gts=gt_cuda,gts_diff_2=gts_diff_2,gts_diff_5=gts_diff_5)  # output = (1, 19, 713, 713)
    ...
    if args.local_rank == 0:
        ...
        #evaluate_eval_for_inference( iou_acc, args.dataset_cls)
