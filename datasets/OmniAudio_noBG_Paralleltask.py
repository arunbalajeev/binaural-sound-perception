...
def make_dataset(quality, mode,  fine_coarse_mult=6, cv_split=0):
    ...
    if mode == 'train':
        for sc in range(1,115):
            ...
            if randomno ==0:
                ...
                audioImg_path = os.path.join(root, img_dir_name, 'spectrograms/Track8/')
                audioImg_path6 = os.path.join(root, img_dir_name, 'spectrograms/Track3/')
                wavaudioImg_path = os.path.join(root, img_dir_name, 'wavsplits/Track8/')
                wavaudioImg_path6 = os.path.join(root, img_dir_name, 'wavsplits/Track3/')
            wavaudioImg_path2 = os.path.join(root, img_dir_name, 'wavsplits/Track1/')
            wavaudioImg_path5 = os.path.join(root, img_dir_name, 'wavsplits/Track6/')
    ...
    if mode == 'val1':
        anno_path = "/srv/beegfs02/scratch/language_vision/data/Sound_Event_Prediction/audio/dataset/outdoor/manual_annotations/"
        img_dir_name = "daylight/";img_dir_name1 = "daylight/"
        
        audioImg_path = os.path.join(anno_path, "Spectrograms/",img_dir_name, 'Track3/');
        audioImg_path6 = os.path.join(anno_path, "Spectrograms/",img_dir_name, 'Track8/')
        audio_postfix='.npy';
        for it_full in glob.glob(audioImg_path+"*.npy"):
            it = it_full.split('/')[-1].split('.')[0]
            #img_path = os.path.join(anno_path,"backgrounds/", img_dir_name,it.split('_')[0]+'_bg.png')
            img_path = os.path.join(anno_path, "Images/", img_dir_name1,'video_'+it.split('_')[0]+'_cropped_'+it.split('_')[1]+'.jpg')
            mask_path = os.path.join(anno_path, "gt_labelIds/", img_dir_name,'video_'+it.split('_')[0]+'_cropped_'+it.split('_')[1]+'.png')
            item = (img_path, mask_path, mask_path, os.path.join(audioImg_path, it+audio_postfix),  os.path.join(audioImg_path6, it+audio_postfix))
            items.append(item)

    if mode == 'val':
        for sc in range(115,140):#(51,60):#(163,166):#(85,99):#(112,119):#(85,99):#(163,166):#(45,60):
            if sc<37:    img_dir_name = 'scene'+str(sc)
            else:    img_dir_name = 'scene%04d'%sc
            check_audioImg_path = os.path.join(root, img_dir_name, 'spectrograms/Track1/')
            audioImg_path = os.path.join(root, img_dir_name, 'spectrograms/Track8/')
...
class OmniAudio(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img, mask, sem_mask, audio1, audio6, audio_channel1, audio_channel6, audio_channel2, audio_channel5 = Image.open(img_path), Image.open(mask_path), Image.open(sem_mask), np.load(audio1), np.load(audio6), np.load(audio_path1), np.load(audio_path6), np.load(audio_path2), np.load(audio_path5)
        ...
        mask = np.array(mask);sem_mask = np.array(sem_mask);#[:,:,0]
        ...
        for k, v in id_to_trainid.items():
            ...
            mask_copy[mask == k] = v
        mask_copy[sem_mask<128] =0;
        ##### Rotate the image ########
        if mod=='val':
            #print("Eval mode")
            mask_rot = mask_copy;sem_mask_rot = sem_mask;
            mask_rot[0:240,:]=mask_copy[240:480,:];mask_rot[240:480,:]=mask_copy[0:240,:];
            sem_mask_rot[0:240,:]=sem_mask[240:480,:];sem_mask_rot[240:480,:]=sem_mask[0:240,:];
            mask_copy = mask_rot; sem_mask = sem_mask_rot;
        ################################
        #pos=np.where(np.array(mask_copy) >20);#print(pos)
        #print(mask_copy[50:60,50:60])
        #print(np.unique(mask_copy))
        if self.eval_mode:
            return [transforms.ToTensor()(img)], self._eval_get_item(img, mask_copy,sem_mask, audio1, audio6,
                                                                     self.eval_scales,
        ...
        audio_diff_spec_5 = torch.FloatTensor(generate_spectrogram(audio_channel1 - audio_channel5))
        audio_diff_spec_2 = torch.FloatTensor(generate_spectrogram(audio_channel6 - audio_channel2))
        ...
        return img, mask,audio_diff_spec_2,audio_diff_spec_5, sem_mask, audio1, audio6, img_name
