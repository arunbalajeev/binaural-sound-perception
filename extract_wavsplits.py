...
for sc in range(1,166):
    ...
    save_dir=fdir+"wavsplits/Track"+str(track)+"/";
    ...
    for i in sorted(glob.glob(fdir+"split_videoframes/*.png")):
        ...
        #print(audio_channel1.shape[0])
        if audio_channel1.shape[0]==96000:
            np.save(save_dir+'%06d.npy'%audio_start_time,audio_channel1)
            #librosa.output.write_wav(save_dir+'%06d.wav'%audio_start_time, audio_channel1, rate)
